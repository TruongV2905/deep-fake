    ...
    <footer className="surface fixed bottom-0 left-0 right-0 z-50 border-t border-white/[0.08] px-4 py-3 sm:px-6">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
        <div className="hidden items-center gap-2 text-sm text-[var(--text-muted)] md:flex">
          <Users className="h-4 w-4" /> Nhóm 4 · MLN111
        </div>
          ...
          {slides.map((slide, i) => (
            <button
              ...
              className={cn(
                ...
                i === currentSlide ? "w-8 bg-[var(--accent)]" : "w-1.5 bg-white/20 hover:bg-white/40",
              )}
        ...
        <div className="hidden text-xs text-[var(--text-muted)] lg:block">Cuộn · Space · ← → · F</div>
      </div>
    </footer>
  );
}
